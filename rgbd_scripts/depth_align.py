...
with pipeline:
    ...
    while True:
        ...
        if key == ord("q"):
            break
        # Format and log the depth and RGB data
        elif key == ord("s"):
            # Format to what we want (aligned float32 depth in meters, RGB uint8)
            log_depth_meters = frameDepth.getFrame().astype(np.float32) / 1000.0
            
            if len(cvFrame.shape) == 2:
                log_rgb = cv2.cvtColor(cvFrame, cv2.COLOR_GRAY2RGB)
            else:
                log_rgb = cv2.cvtColor(cvFrame, cv2.COLOR_BGR2RGB)
            
            # Save to disk as a compressed numpy array
            timestamp = int(time.time())
            filename = f"scan_frame_{timestamp}.npz"
            np.savez_compressed(
                filename, 
                rgb=log_rgb, 
                depth=log_depth_meters
            )
            print(f"[{timestamp}] Successfully logged formatted RGB-D data to {filename}")
